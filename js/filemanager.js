var FileManager=(function(){'use strict';
...
function iconFor(n){var t=typeFromName(n);return t==='html'?'🌐':t==='image'?'🖼️':t==='js'?'⚙️':t==='css'?'🎨':'📄';}
function truncCid(c){if(!c||c.length<16)return c;return c.slice(0,8)+'…'+c.slice(-6);}
function gwUrl(c){return PinataAPI.GATEWAY_URL+c;}
...
function loadFiles(){var key=getActiveKey();if(!key)return;showLoading(true);
PinataAPI.listPins({apiKey:key.key,sort:'DESC',limit:100}).then(function(data){allPins=data.pins;applyFilters();showLoading(false);loadStorage(key.key);}).catch(function(){allPins=[];applyFilters();showLoading(false);});}
function loadStorage(apiKey){PinataAPI.getUsage(apiKey).then(function(usage){var pct=usage.percent;els.storageText.textContent=fmt(usage.totalSize)+' of '+fmt(usage.maxSize);els.storageFill.style.width=Math.min(pct,100)+'%';els.storageFill.className='storage-fill'+(pct>=90?' danger':pct>=70?' warn':'');if(pct>=90){els.storageWarning.style.display='block';}}).catch(function(){els.storageText.textContent='Unable to calculate';});}
function applyFilters(){var pins=allPins.slice();
...
if(currentFilter!=='all'){pins=pins.filter(function(p){return typeFromName(p.name)===currentFilter;});}
...
updateBulkBar();}
function renderGrid(){els.fmGrid.innerHTML='';filteredPins.forEach(function(pin){
var card=document.createElement('div');card.className='fm-card';card.setAttribute('data-cid',pin.cid);
card.innerHTML='<div class="fm-card-top"><input type="checkbox" class="fm-card-check" data-cid="'+pin.cid+'"'+(selectedCids.has(pin.cid)?' checked':'')+'><div class="fm-card-icon">'+iconFor(pin.name)+'</div><div class="fm-card-info"><div class="fm-card-name" title="'+pin.name+'">'+pin.name+'</div><div class="fm-card-cid" title="'+pin.cid+'">'+truncCid(pin.cid)+'</div></div></div><div class="fm-card-meta"><span>'+fmt(pin.size)+'</span><span>'+fmtDate(pin.date)+'</span><span class="fm-card-badge active">Active</span></div><div class="fm-card-actions"><button class="btn-icon-sm fm-act" data-act="ipfs" data-cid="'+pin.cid+'" title="Copy IPFS link">📋</button><button class="btn-icon-sm fm-act" data-act="gw" data-cid="'+pin.cid+'" title="Copy Gateway link">🔗</button><a class="btn-icon-sm fm-act" href="'+gwUrl(pin.cid)+'" target="_blank" rel="noopener" title="Open in new tab">↗</a><button class="btn-icon-sm fm-act" data-act="versions" data-name="'+pin.name+'" title="Versions">🕐</button><button class="btn-icon-sm fm-act fm-btn-danger" data-act="unpin" data-cid="'+pin.cid+'" title="Unpin">✕</button></div>';
els.fmGrid.appendChild(card);});}
function renderList(){els.fmListBody.innerHTML='';filteredPins.forEach(function(pin){
var tr=document.createElement('tr');
tr.innerHTML='<td class="fm-td-check"><input type="checkbox" data-cid="'+pin.cid+'"'+(selectedCids.has(pin.cid)?' checked':'')+'></td><td><div class="fm-td-name">'+iconFor(pin.name)+' <span title="'+pin.name+'">'+pin.name+'</span></div></td><td>'+fmt(pin.size)+'</td><td>'+fmtDate(pin.date)+'</td><td>'+typeFromName(pin.name).toUpperCase()+'</td><td><div class="fm-td-actions"><button class="btn-icon-sm fm-act" data-act="ipfs" data-cid="'+pin.cid+'">📋</button><button class="btn-icon-sm fm-act" data-act="gw" data-cid="'+pin.cid+'">🔗</button><a class="btn-icon-sm" href="'+gwUrl(pin.cid)+'" target="_blank" rel="noopener">↗</a><button class="btn-icon-sm fm-act" data-act="versions" data-name="'+pin.name+'">🕐</button><button class="btn-icon-sm fm-act fm-btn-danger" data-act="unpin" data-cid="'+pin.cid+'">✕</button></div></td>';
els.fmListBody.appendChild(tr);});}
function setView(view){currentView=view;localStorage.setItem('fm_view',view);
...
function confirmUnpinAction(){if(!unpinTarget)return;var key=getActiveKey();if(!key)return;var cid=unpinTarget;closeUnpinModal();
PinataAPI.unpin(cid,key.key).then(function(){allPins=allPins.filter(function(p){return p.cid!==cid;});selectedCids.delete(cid);applyFilters();}).catch(function(err){alert(err.message||'Failed to unpin');});}
...
function confirmBulkUnpinAction(){var key=getActiveKey();if(!key)return;var cids=Array.from(selectedCids);closeBulkUnpinModal();
PinataAPI.unpinBulk(cids,key.key).then(function(res){res.success.forEach(function(cid){allPins=allPins.filter(function(p){return p.cid!==cid;});selectedCids.delete(cid);});applyFilters();if(res.failed.length>0)alert(res.failed.length+' files failed to unpin.');});}
...
function confirmRollbackAction(){if(!rollbackTarget)return;
var url=gwUrl(rollbackTarget);window.open(url,'_blank');closeRollbackModal();}
function startCompare(){compareMode=true;compareSelected=[];if(els.vpCompareBar)els.vpCompareBar.style.display='flex';if(els.vpCompareCount)els.vpCompareCount.textContent='0 selected';
...
renderVersionList(versions);}
function cancelCompare(){compareMode=false;compareSelected=[];if(els.vpCompareBar)els.vpCompareBar.style.display='none';
var name=els.vpSiteName.textContent;var versions=allPins.filter(function(p){return p.name===name||p.name.replace(/-v\d+$/,'')===name.replace(/-v\d+$/,'');}).sort(function(a,b){return new Date(b.date)-new Date(a.date);});
renderVersionList(versions);}
function toggleCompareVersion(cid){var idx=compareSelected.indexOf(cid);if(idx!==-1)compareSelected.splice(idx,1);else{if(compareSelected.length>=2)compareSelected.shift();compareSelected.push(cid);}
...
document.querySelectorAll('.fm-sortable').forEach(function(th){th.addEventListener('click',function(){var col=th.getAttribute('data-col');
...
else if(col==='type')filteredPins.sort(function(a,b){return typeFromName(a.name).localeCompare(typeFromName(b.name));});
...
else if(act==='gw')copyText(gwUrl(cid),actBtn);
...
return{init:init,loadFiles:loadFiles};
})();
